...
def shell_return(shell_cmd, suppress=False):
    ...
    return subprocess.run(
        shell_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    ).returncode
...
def create_unit_test(functions, file_size=4096, max_file_size=8192, num_ops=8192):
    def unit_test(infile, outfile, outfile2):
        return (
            shell_return(
                f"./io300_test {functions} -s {file_size} -m {max_file_size} -n {num_ops}"
            )
            == 0
        )
...
def byte_cat(infile, outfile, outfile2):
    return shell_return(f"./byte_cat {infile} {outfile}") == 0 and files_same(
        infile, outfile
    )
...
def diabolical_byte_cat(infile, outfile, outfile2):
    return shell_return(
        f"./diabolical_byte_cat {infile} {outfile}"
    ) == 0 and files_same(infile, outfile)


def reverse_byte_cat(infile, outfile, outfile2):
    return (
        shell_return(f"./reverse_byte_cat {infile} {outfile}") == 0
...
def create_block_cat(block_size):
    def block_cat(infile, outfile, outfile2):
        return shell_return(
            f"./block_cat {block_size} {infile} {outfile}"
        ) == 0 and files_same(infile, outfile)
...
def create_reverse_block_cat(block_size):
    def reverse_block_cat(infile, outfile, outfile2):
        return (
            shell_return(f"./reverse_block_cat {block_size} {infile} {outfile}") == 0
            and not files_same(infile, outfile)
            and shell_return(f"./reverse_block_cat {block_size} {outfile} {outfile2}")
            == 0
            and files_same(infile, outfile2)
        )
...
def random_block_cat(infile, outfile, outfile2):
    return shell_return(f"./random_block_cat {infile} {outfile}") == 0 and files_same(
        infile, outfile
    )
...
def rot13(infile, outfile, outfile2):
    ...
    return (
        shell_return(f"./rot13 {outfile}") == 0
...
def runtests(tests, suite_name):
    ...
    assert (
        shell_return(f"dd if=/dev/urandom of={infile} bs=4096 count=20", suppress=True)
        == 0
    )
    ...
    for i, test in enumerate(tests):
        ...
        if test == "ascii_independence":
            ...
            f.write(
                "Make\x00sure\x00your\x00cache\x00can\x00handle\x00null\x00bytes! 가정하는 것은 안전하지 않습니다 प्रत्येकं पात्रं इति 'n ASCII-karakter."
            )
            f.close()
            assert shell_return(f"touch man_integrity", suppress=True) == 0
            assert (
                shell_return(f"cp man_nonascii.txt man_integrity", suppress=True) == 0
            )
            ...
            assert (
                shell_return(f"rm -- man_nonascii.txt man_integrity", suppress=True)
                == 0
            )
        else:
    ...
    log(
        "If a test failed, you should debug with a small sample file to see how your output is different!"
    )
    ...
    assert (
        shell_return(f"rm -- {infile} {outfile} {outfile2} {integrity}", suppress=True)
        == 0
    )
...
runtests(
    {
        ...
        "seek_beyond_eof": create_unit_test(
            "readc writec seek", file_size=0, max_file_size=4096, num_ops=1000
        ),
