...
def time_program(progcmd):
    ...
    perf_data = {
        ...
        "wtime": get_time_field(
            time_output, "Elapsed (wall clock) time (h:mm:ss or m:ss):"
        ),
...
def runtests(tests):
    ...
    for testname in tests:
        if type(results[testname]["stdio"]) is not dict:
            print(
                "ERROR: stdio program failed. This should not happen, please contact the course staff"
            )
        ...
        log(
            "performance result: %s: stdio=%.2fs, student=%.2fs, ratio=%.2f"
    ...
    if "--grader" not in sys.argv:
        for test, ratio in metrics.items():
            if ratio == "student test failed":
                ...
                print(
                    "{}: \033[32;1m{}x\033[0m stdio's runtime".format(
                        test, round(ratio, 2)
                    )
                )
            elif ratio <= 5.0:
                ...
                print(
                    "{}: \033[31;1m{}x\033[0m stdio's runtime".format(
                        test, round(ratio, 2)
                    )
                )
    else:
